...
if (isset($_POST["editAcc"])){
    ...
    if ($editProfImgOk && $editNameOK && $editPhoneOK && $editPwdOK && $editPwdRepeatOK){
        editAccount($conn,$editEmail,$editName,$editPhone,$editPwd);
        if($editProfImgOption == "UpdateProfImg"){
            ...
            $query = "UPDATE custdata SET custProfilePicStatus = 1 WHERE custEmail = '$editEmail';";
            $result = mysqli_query($conn,$query);
        } 
        if($editProfImgOption == "RemoveProfImg"){
            removeCustProfilePic($loggedID);
            $query = "UPDATE custdata SET custProfilePicStatus = 0 WHERE custEmail = '$editEmail';";
            $result = mysqli_query($conn,$query);
        } 
